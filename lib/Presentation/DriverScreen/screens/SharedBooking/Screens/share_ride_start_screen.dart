...
class _ShareRideStartScreenState extends State<ShareRideStartScreen>
  ...
  Future<void> _maybeFillLocalEta(LatLng driverPos, SharedRiderItem active) async {
    ...
    // Instant refresh: straight-line distance + a rough ETA (~20 km/h urban).
    setLeg(straight, straight / 333.0);
    ...
    try {
      ...
      final dMin = (info['durationMin'] as num?)?.toDouble() ?? 0.0;
      final dMeters =
          (info['distanceToDestinationMeters'] as num?)?.toDouble() ?? 0.0;
      setLeg(dMeters > 0 ? dMeters : straight, dMin > 0 ? dMin : straight / 333.0);
    } catch (_) {
